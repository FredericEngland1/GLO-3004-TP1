COMMENT EXECUTER LE CODE :
    Nous fournissons un executable .jar avec le code, mais certain membre de notre équipe avait des problème pour l'exécuter.
    C'est pourquoi nous fournissons le code au cas où les correcteurs aurait aussi des problèmes avec l'exécutable.

    En théorie il suffit simplement de compiler le code Java et de lancer l'application résultante pour exécuter le code. L'utilisation
    d'un IDE est recommandé car cela simplifie le processus. Nous assumons que le correcteur à des connaissances de base en programmation
    et sera en mesure d'exécuter les code avec les étape ci-dessus.
    Il est possible que l'utilisateur ait besoin d'avoir les librairies de Swing pour Java pour que l'interface fonctionne correctement.

AVANT DE COMMENCER :
    Avant de démarrer la Confiturerie, il est important de bien sélectionner le nombre de bocaux, valves et étiqueteuses qui seront utilisés.
    Ces nombres correspondent à N, V et E respectivement. Il est aussi recommandé de mettre un temps de sommeil (millisecondes) relativement long
...
POUR COMMENCER :
    Une fois les variables de base établies, on peut débuter la simulation avec le bouton "Commencer la simulation" (La modification des variables autre
    que le temps de sommeil ne sera pas prise en compte a moins d'arrêter la Confiturerie et de la redémarrer).

POUR ARRETER :
    Une fois la simulation débutée, il est possible de l'arrêter complètement avec le bouton "Arret". Au redémarrage, le console du UI sera vidé.

PENDANT LA SIMULATION :
    À n'importe quel moment pendant la simulation, il est possible de la mettre sur pause en appuyant sur le bouton "Pause".
    Pour reprendre la simulation il suffit d'appuyer sur le bouton "Redemarrer". Il est à noter que si le bouton de pause
...
RUPTURE ET APPROVISIONNEMENT :
    Pour faciliter le travail du correcteur (oui toi!) nous avons décidé que la rupture et l'approvisionnement allaient être contrôlés
    ...
    le type de bocal à approvisionner de la même manière que pour la rupture. Il suffit ensuite d'appuyer sur le bouton "Approvisionnement".
...
COMMENT INTERPRETER LES SORTIES :
    ...
    accomplies par les processus au cours de leur exécution. Ces lignes aurons la forme :
...
TRUCS ET ASTUCES :
    Pour faciliter le contrôle de la trace, il est recommandé de mettre un temps de sommeil suffisamment long pour laisser le temps à
    ...
    ou un approvisionnement, il est recommandé de d'abord faire pause et de s'assurer qu'après l'exécution des processus en cours,
    le moment est toujours le bon, de créer la rupture ou l'approvisionnement, puis de redémarrer les processus. Il est à noter qu'il
